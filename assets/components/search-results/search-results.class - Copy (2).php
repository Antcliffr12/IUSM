...
class SearchResults{
        ...
        if(!is_null($this->version) && $this->is_news_search()){
          ...
          case '0':
            ...
            'meta_query' => array(
              ...
              $arrayCampus = $this->GetCampusData();
              //$this->results = $this->GetCampusData();
              $this->results = array_merge($this->search($arrayCampus, 'title', $search_string), $this->search($arrayCampus, 'description', $search_string));
              break;
          }//switch
    ...
    private function GetCampusData(){
      $search_string = $_GET['s'];
      if($search_string == "Bloomington"){
        $feed = 'https://news.iu.edu/rss?c=iub';
      }elseif($search_string=="IU East") {
        $feed = "https://news.iu.edu/rss?c=iue";
      }
      elseif($search_string=="Fort Wayne") {
        $feed = "https://news.iu.edu/rss?c=ipfw";
      }
      elseif($search_string=="Indianapolis") {
        $feed = "https://news.iu.edu/rss?c=iupui";
      }
      elseif($search_string=="Muncie") {
        $feed = "https://news.iu.edu/rss?c=iupui";
      }
