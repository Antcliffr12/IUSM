...
EOQ;
	...
	public static function getResidentListByDepartment($department_code)
	{
		$q = <<<EOQ
SELECT
	DISTINCT lc.Prsn_Univ_Id AS uid,
		lc.PersonId AS pid,
		lc.FullName AS display_name,
		lc.FirstName AS first_name,
		lc.LastName AS last_name,
		lc.MiddleName AS middle_name,
		lc.PersonStatus AS status,
		lc.PersonType AS utype,
		lc.UserName AS username,
		lc.PhoneNumber AS phone,
		lc.Fax AS fax,
		lc.TrainingPrgDescr + ', PGY ' + pgy AS trainingDesc,
		lc.fromSchool AS school,
		lc.high_DegreeCd as highDegree,
		lc.EmailAddress AS email,
		lc.Position_Title AS positions,
		lc.Job_DepartmentCode AS dept_code,
		lc.Job_DepartmentDescr AS dept_desc
	FROM vwSec_LearnerCore lc
WHERE Job_DepartmentCode  = '{$department_code}'
ORDER BY lc.LastName, lc.FirstName
EOQ;
