...
JOIN vwSec_FacultyBio fb ON fc.PersonId = fb.PersonId
...
EOQ;
		...
		// Education degree(s)
		if (!$this->mock) {
			$queryDegree = self::query("SELECT Degree FROM vwSec_FacultyEducation WHERE Prsn_Univ_Id = '{$this->uid}' ORDER BY HighestDegree DESC, DegreeRank ASC");
			foreach ($queryDegree as $item) {
				$this->degrees[] = $item['Degree'];
			}
		} else {
			$this->degrees = ['MD', 'MBA'];
		}
...
EOQ;
		...
		if (count($results) === 0) {
			...
			foreach ($results as $row) {
				...
				//education degree
				$qe = "SELECT Degree FROM vwSec_FacultyEducation WHERE Prsn_Univ_Id = '" . $row['uid'] . "' ORDER BY HighestDegree DESC, DegreeRank ASC";
				$queryEducation = odbc_exec($odbc_conn, $qe);

				$degrees = [];

				while ($rowEducation = odbc_fetch_array($queryEducation)) {
					$degrees[] = $rowEducation['Degree'];
				}
...
WHERE fl.ACAD_TTL_DEPT_ID = '{$department_code}' AND fl.PRSN_STAT_CD = 'A' AND {$cond}
...
EOQ;
	...
	public static function getListBySubDepartment($department_code, $division_code, $category = 1)
	{
		$odbc_conn = odbc_connect($_SERVER['DNS_NAME'], $_SERVER['HR_DB_USER'], $_SERVER['HR_DB_PASSWORD']);
		$data = array();
		if ($category == 0) {
		...
		FROM vwSec_FacultyCore fc
		JOIN vwSec_FacultyListings fl ON fc.Prsn_Univ_Id = fl.Prsn_Univ_Id
		WHERE fl.ACAD_TTL_DEPT_ID = '{$department_code}' AND fl.PRSN_STAT_CD = 'A' AND fc.job_departmentcode = '{$division_code}' AND {$cond}
		ORDER BY fc.LastName, fc.FirstName
