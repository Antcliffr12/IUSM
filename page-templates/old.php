<?php
/*  Template Name: Event In Page Template Old */
...
<script type="text/javascript" src="<?php bloginfo("template_url"); ?>/assets/scripts/jquery.min.js"></script>
  ...
  //console.log($this.find('event'));
    ...
    trys = trys.replace(regExString, "") //find all tags and delete them.
      ...
      '<div class="featured-event-date flex-vertical-spacing" data-sdical='+ encodeURI(eventList[i]['start-date-time']) + ' data-edical='+ encodeURI(eventList[i]['end-date-time']) + ' > '+ day + eventList[i]['start-date-time'].substring(16,19) + ' - ' + eventList[i]['end-date-time'].substring(10,17) + eventList[i]['end-date-time'].substring(16,19) +' </div>'+
			...
			//$(eventList[i]['description']).text().trim() +
       decodeURI(trys).substring(0,165) +' ...'+'<a  href="<?php echo esc_url( get_permalink( get_page_by_title( 'More Event' ) ) ); ?>?eventId='+eventList[i]['event-id']+'"> MORE </a> '+
...
$(".menu-item-63").click(function(){ // event-today
  $('.btn-wrap ').show();
  todayDateEvent();
  hideButton();
...
$(".menu-item-64").click(function(){ //events-this-week
  $('.btn-wrap ').show();
  getWeekEvents();
  hideButton();
});
...
$(".menu-item-66").click(function(){ //.events-all
  ShowAllEvents();
});
...
function callingXMLData(date, isAllEvents, specificUrl){
    ...
    var XMLData = null,
                    ...
                    m = d.getMonth(),
                    y = d.getFullYear(),
                    sDate = (new Date(y,m,1).toISOString().slice(0,10)).toString(); //this is first day of current month
  ...
  return $('<div/>').html(value).text();
	...
	var eventDiv ='<div class="upcoming-event">'+
                        ...
                        '<div class="event-description">'+ htmlDecode(description.substring(0,300))
                           +'....'+ '<a id="read_more" href="<?php echo esc_url( get_permalink( get_page_by_title( 'More Event' ) ) ); ?>?eventId='+eventObject['event-id']+'">MORE</a> '+
                         '</div>'
...
function hideEvents(){
	...
	if(totallicat > numberperload){
		...
		for (i = numberperload; i < totallicat; i++) {
										$(categorylist[i]).hide();
		}
	}
