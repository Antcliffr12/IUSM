...
$faculty_id = filter_var(get_query_var('faculty_id'), FILTER_SANITIZE_NUMBER_INT);
...
if (count($segments) == 5 && $segments[0] == 'departments' && $segments[2] == 'faculty' && is_numeric($segments[3]) ) {
	$section_switcher = render_component('department-switcher', ['path' => $path]);
	$subnav_root_path = '/departments/' . $section;
	$parent = get_page_by_path("/departments/$section/faculty");
	$post = $parent->ID;
} else if (count($segments) == 5 && $segments[0] == 'campuses' && $segments[2] == 'faculty' && is_numeric($segments[3])) {
	$section_switcher = render_component('campus-switcher', ['path' => $path]);
	$subnav_root_path = '/campuses/' . $section;
	$parent = get_page_by_path("/campuses/$section/faculty");
	$post = $parent->ID;
} else if (count($segments) == 3 && $segments[0] == 'faculty' && is_numeric($segments[1])) {
	$hide_sidebar = true;
}
...
// Instantiate a FacultyData object for the specified person
$profile = new IU\FacultyData($faculty_id);
	...
	<div class="container">    
		<div class="row">
			...
			<div id="region-main" class="col-xs-12 <?=($hide_sidebar)? 'col-md-12' : 'col-md-9'?>">
				...
				<?= render_component('faculty-profile', [
					'profile' => $profile,
					'fullname' => $fullname,
					'title' => $title,
				]); ?>
				...
				<section class="iu-section-block bg-white padding-normal padding-condensed-top">
                    ...
                    <div class="container">
						<aside class="iu-callout-box callout-normal bg-iu-cream" data-component="Callout Box">
							...
							<div class="button-row">
								<a class="button" data-style="default" href="https://medicine.iu.edu/research/labs/faculty/">Faculty Labs</a>
							</div>
						</aside>
					</div>
	                <?php endif; ?>
